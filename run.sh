...
# ----------------------------------------------------------------
# PHASE 0: Authentication and Resource Discovery (Hardened Logic)
# ----------------------------------------------------------------
...
echo "0.2. Identifying attached volumes on LPAR: $LPAR_NAME"
...
# ----------------------------------------------------------------
# PHASE 1: Immediate Shutdown and Poll for SHUTOFF (Updated logic)
# ----------------------------------------------------------------
...
if [[ "$LPAR_STATUS" != "SHUTOFF" ]]; then
    echo "LPAR status is $LPAR_STATUS. Initiating **immediate-shutdown** operation to enforce required SHUTOFF state."
    
    # Execute the immediate-shutdown operation (hard stop) [1, 2]
    ...
    if [[ "$LPAR_STATUS" != "SHUTOFF" && "$LPAR_STATUS" != "ERROR" && "$LPAR_STATUS" != "NOT_FOUND" ]]; then
        echo "FATAL: LPAR shutdown timed out. Volumes cannot be safely detached or deleted while LPAR is potentially ACTIVE."
        exit 1
    fi
else
...
# ----------------------------------------------------------------
# PHASE 2: Detach DATA Volume(s) and Poll for Detachment (REQUIRED FIRST)
# ----------------------------------------------------------------
if [[ -n "$CLONE_DATA_IDS" ]]; then
    echo "2. Detaching Data Volume(s) first: $CLONE_DATA_IDS"
    # Use ibmcloud pi instance volume bulk-detach [3-5]
    # Set --detach-primary False as we are only handling data volumes here
    ...
    while [[ $ITERATIONS -lt 15 ]]; do 
        # Check attached volumes list (only non-bootable volumes)
...
# ----------------------------------------------------------------
# PHASE 3: Detach BOOT Volume and Poll for Detachment (REQUIRED SECOND)
# ----------------------------------------------------------------
if [[ -n "$CLONE_BOOT_ID" ]]; then
    echo "3. Detaching Boot Volume: $CLONE_BOOT_ID"
    # Use individual detach command (ibmcloud pi instance volume detach) [6, 7]
    ...
    while [[ $ITERATIONS -lt 10 ]]; do 
        # Check if the boot volume ID is still listed as attached (only bootable volumes)
...
# ----------------------------------------------------------------
# PHASE 4: Snapshot Discovery and Deletion (Logic unchanged)
# ----------------------------------------------------------------
...
if [[ -n "$ALL_CLONE_IDS" ]]; then
    ...
    # Volumes must be detached before deletion can proceed [8].
    # Use ibmcloud pi volume bulk-delete [3, 9, 10]
